...
comment on column app_pedido_item.preco_unitario is
    'Preço unitário decidido para este item. Nasce de VL_ENT_UNIT (COMPRAS_PEDIDO) — o valor da última entrada NA NOTA, bruto de imposto/crédito e já por unidade (REGRAS.md regra 8) — no momento em que o produto entra no pedido, e é editável em tela enquanto o pedido está em Rascunho/Orçamento Enviado. ATÉ 24/09/2026 nascia de CUSTO_TOT_GERENCIAL (líquido), como PROTOTIPO.md §3.7 ainda descreve; mudou a pedido do usuário, para o comprador negociar contra o preço que o fornecedor de fato cobrou. Pedidos gravados ANTES dessa data mantêm o preço antigo — o valor é um snapshot da linha, não recalculado. É este valor, não um preço recalculado depois, que vai na coluna de preço do arquivo de exportação Winthor. CHECK > 0.';
