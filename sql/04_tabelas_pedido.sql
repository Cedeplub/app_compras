...
comment on column app_pedido_item.preco_unitario is
    'Preço unitário decidido para este item. Desde 25/09/2026 nasce de PRECO_ULT_ENT_SEM_FRETE (COMPRAS_PRODUTO_CONTEXTO) — o preço da última entrada pela rotina 218, SEM frete e já por unidade (REGRAS.md regra 8) — no momento em que o produto entra no pedido, e é editável em tela enquanto o pedido está em Rascunho/Orçamento Enviado. QUEDA para VL_ENT_UNIT (COMPRAS_PEDIDO, valor da nota COM frete) quando o produto não tem valor da 218 (medido em 25/09/2026: 41 SKUs ATIVOS nesse caso) — nesses itens o preço gravado sai com frete, exceção documentada, não falha silenciosa. ATÉ 24/09/2026 nascia de VL_ENT_UNIT sempre (com frete); ANTES disso, de CUSTO_TOT_GERENCIAL (líquido), como PROTOTIPO.md §3.7 ainda descreve. Pedidos gravados antes de cada mudança mantêm o preço com que foram gravados — o valor é um snapshot da linha, não recalculado. É este valor, não um preço recalculado depois, que vai na coluna de preço do arquivo de exportação Winthor. CHECK > 0.';
