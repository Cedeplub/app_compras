...
def _filtros(f: dict, alias: str = "m") -> tuple[str, dict]:
    ...
    if f.get("comprador"):
        # Nem COMPRAS_MONITORAMENTO nem COMPRAS_ENTRADA têm coluna COMPRADOR —
        # o departamento aqui é a mesma stg_departamento.descricao_departamento
        # que alimenta COMPRAS_IND_FORNECEDOR.FORNECEDOR, então o join resolve
        # sem mudança no dbt. Medido em 08/09/2026: os 47 departamentos de
        # monitoramento e os 44 de entrada casam 100% com ind_fornecedor —
        # nenhum SKU some do filtro por falta de match.
        cond.append(
            f"exists (select 1 from compras_ind_fornecedor cif"
            f" where cif.fornecedor = {alias}.departamento"
            f" and cif.comprador = :comprador)"
        )
        binds["comprador"] = f["comprador"]
...
def opcoes_monitoramento() -> dict:
    ...
    Exceção deliberada: `compradores` reaproveita `produto.opcoes()`, que é o
    catálogo INTEIRO de compradores, não só os com movimento — uma fonte só
    para "quem são os compradores" vale mais do que a garantia de "só quem
    tem movimento" aqui (ver comentário abaixo, na montagem do dict).
    """
    ...
    return {
        ...
        # Mesma lista de `produto.opcoes()` — uma fonte só para "quem são os
        # compradores", em vez de uma segunda consulta que pode divergir.
        "compradores": produto.opcoes()["compradores"],
...
def entradas(filtros: dict, de: str, ate: str, limite: int = 200) -> dict:
    ...
    if filtros.get("comprador"):
        cond.append(
            "exists (select 1 from compras_ind_fornecedor cif"
            " where cif.fornecedor = e.departamento and cif.comprador = :comprador)"
        )
