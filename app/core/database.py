...
# Todo model do dbt e "table" full-refresh (nao ha incremental nenhum): durante
# o `dbt run`, ~50 objetos sao trocados por renomeacao (__dbt_tmp -> nome ->
# __dbt_backup), e em cada troca existe uma janela em que o nome nao resolve.
# Com carga automatica as 06:00/13:00 e botao manual, essa janela deixa de ser
# hipotese e vira 500 na cara de quem esta com a tela aberta. Estes tres (e
# so estes tres) codigos sao sintoma dessa troca de tabela, nao de erro de
# SQL ou de dado: uma unica retentativa apos a janela tipicamente fechar
# resolve; se persistir, e problema de verdade e deve subir.
_ORA_TABELA_EM_TROCA = ("ORA-00942", "ORA-08103", "ORA-04043")
...
def consultar(sql: str, binds: dict | None = None, timeout: int | None = None) -> list[dict]:
    ...
    try:
        ...
        if "DPY-4011" in str(exc) or "DPY-3015" in str(exc):
            raise
        if any(codigo in str(exc) for codigo in _ORA_TABELA_EM_TROCA):
            log.warning("consulta pegou tabela em troca pelo dbt, tentando de novo em 1.5s: %s", exc)
            time.sleep(1.5)
            try:
                with conexao(timeout) as conn:
                    cur = conn.cursor()
            ...
            except oracledb.Error as exc2:
                log.warning("consulta falhou de novo apos retentativa: %s", exc2)
                raise
