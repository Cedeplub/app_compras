...
import { parseNumeroPreco } from "./formato.js";

const nulo = (v) => v === null || v === undefined || Number.isNaN(v);

/** Tolerância para considerar dois preços "o mesmo preço" ao decidir se um
 *  valor redigitado é alteração de verdade — não vem do servidor: é o FRONT
 *  quem evita tratar ponto flutuante (ou o mesmo valor batido de novo) como
 *  uma nova decisão. Preço tem 2 casas decimais, então qualquer diferença
 *  real é >= 0,01; meio centavo cobre o arredondamento sem aceitar um
 *  centavo de diferença de verdade. Usada em `Precificacao.jsx` e
 *  `DecisaoSKU.jsx` — era 0,005 solto em três lugares antes desta constante.
 */
...
export function simular({ produto, cenarios, cenarioSel, precoAtual, precoDigitado,
  ...
  // `precoDigitado` é o texto do campo — ver o contrato de `parseNumeroPreco`
  // em formato.js. `Number.isFinite` descarta os dois casos que não valem
  // como "preço novo": vazio (parser devolve `null` — ainda não decidiu) e
  // texto que não é número (`NaN`). Zero passa no `isFinite`, mas não é
  // preço válido — por isso o `> 0` continua aqui, como antes.
  const parsed = parseNumeroPreco(precoDigitado);
  const novo = Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
