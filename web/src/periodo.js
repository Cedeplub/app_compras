...
/** Data ISO `dias` antes da referência, em UTC.
 *
 *  Usado pelos atalhos "sem entrada há mais de N dias" (Precificação e
 *  Pedidos, item 1 do Diretor de 08/09/2026): a pergunta real não é "o que
 *  entrou entre duas datas", é "o que não entra há tempo demais" — então o
 *  atalho calcula só o limite superior (`dtUltEntAte`) e deixa o inferior em
 *  aberto. A referência é a DATA DO DADO, nunca o relógio do navegador, pelo
 *  mesmo motivo do resto deste arquivo. */
export function diasAntes(referenciaISO, dias) {
  const d = dataDeISO(referenciaISO);
