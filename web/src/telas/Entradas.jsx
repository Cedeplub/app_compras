...
export default function Entradas() {
  ...
  // teto é do SERVIDOR — `entradas()` (`app/servicos/monitoramento.py:346-347`)
  // corta `rownum <= 200` DEPOIS de `order by data_entrada desc, valor desc`
  // — as 200 entradas mais recentes do filtro (empate de data desempatado
  // pelo maior valor), não "as 200 primeiras" de nenhuma outra ordem.
  // Reordenar no cliente por outra coluna (ex.: "Valor") só reordena esse
  // recorte de 200, nunca traz de volta uma entrada mais antiga que ficou de
  // fora — o rótulo acima da lista (abaixo) diz isso, para a ordenação local
  // não parecer mais completa do que é.
  ...
  }, [periodo, departamento, secao, comprador, busca, versaoDados]);
  ...
  return (
    <div className="px-4 pb-6 pt-3 md:px-6 md:pt-4">
      ...
      <div className="mt-3 flex flex-wrap items-end gap-3">
        ...
        {/* Backend já pronto: GET /api/entradas aceita `comprador`, e
      ...
      <div className="mt-4">
        ...
        {/* Ajuste 3 do revisor (13/09): `dados.linhas` é a contagem TOTAL do
            filtro (query sem `rownum`); `dados.itens` é o recorte de até 200
            que a lista abaixo mostra e ordena. Só aparece quando o corte de
            fato aconteceu — filtro estreito o bastante para caber tudo em
            200 não precisa do aviso. */}
        {!carregando && !erro && dados && dados.linhas > (dados.itens?.length ?? 0) && (
          <p className="mb-2 text-xs text-gray-500">
            Mostrando as {numero(dados.itens.length)} entradas mais recentes do filtro
            (de {numero(dados.linhas)} no total) — empate de data desempatado pelo maior
            valor. Reordenar as colunas abaixo reordena só este recorte.
          </p>
        )}

        {grupos.map(([titulo, itensGrupo]) => {
          const extratores = {
            data: (it) => it.data, codigo: (it) => it.codigo, nome: (it) => it.nome,
            departamento: (it) => it.departamento, tipo: (it) => it.tipo,
            quantidade: (it) => it.quantidade, precoUnitario: (it) => it.precoUnitario, valor: (it) => it.valor,
          };
          const itens = ordenarLista(itensGrupo, extratores[ordenar] ?? extratores.data, dir);
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 text-2xs uppercase tracking-wide text-gray-500">
                    <CabecalhoOrdenavel {...props} coluna="data" padrao="desc">Data</CabecalhoOrdenavel>
                    <CabecalhoOrdenavel {...props} coluna="codigo" padrao="asc">Código</CabecalhoOrdenavel>
                    <CabecalhoOrdenavel {...props} coluna="nome" padrao="asc" className="min-w-[220px]">Produto</CabecalhoOrdenavel>
                    <CabecalhoOrdenavel {...props} coluna="departamento" padrao="asc">Departamento</CabecalhoOrdenavel>
                    <CabecalhoOrdenavel {...props} coluna="tipo" padrao="asc" align="center">Tipo</CabecalhoOrdenavel>
                    <CabecalhoOrdenavel {...props} coluna="quantidade" padrao="desc" align="right">Qtd.</CabecalhoOrdenavel>
                    <CabecalhoOrdenavel {...props} coluna="precoUnitario" padrao="desc" align="right">Preço unit.</CabecalhoOrdenavel>
                    <CabecalhoOrdenavel {...props} coluna="valor" padrao="desc" align="right">Valor</CabecalhoOrdenavel>
                  </tr>
                </thead>
                <tbody>
                  {itens.map((it, i) => (
