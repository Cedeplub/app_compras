...
export default function Monitoramento() {
  ...
  }, [periodo, anterior, metrica, departamento, secao, comprador, status, porProduto, versaoDados]);
  ...
  return (
    <div className="px-4 pb-6 pt-3 md:px-6 md:pt-4">
      ...
      <div className="mt-3 flex flex-wrap items-end gap-3">
        ...
        {/* Backend já pronto: GET /api/monitoramento aceita `comprador`, e
            GET /api/monitoramento/opcoes devolve a mesma lista de
            /api/opcoes — fonte única. Diferente de Seção, este filtro filtra
            de verdade: 61 departamentos, cada um com exatamente um comprador,
            zero SKU sem comprador (medido em 08/09/2026). */}
        <Campo rotulo="Comprador" largura="w-40">
          <Select valor={comprador} vazio="Todos" opcoes={opcoes?.compradores ?? []}
                  aoTrocar={setComprador} />
        </Campo>
      ...
      <div className="mt-4">
        ...
        {!carregando && !erro && dados && (
          porProduto
            ? <TabelaProdutos itens={dados.produtos ?? []} fmt={fmt} metrica={metrica}
...
function TabelaGrupos({ grupos, dimensao, fmt, aoFixar }) {
  // Etapa 13, ponto 4: lista SEM paginação — ordena NO CLIENTE (§3.1), com o
  // mesmo componente `CabecalhoOrdenavel` das listas paginadas, só que sem
  // ida ao servidor. Ajuste 3 do revisor (13/09): diferente de
  // `TabelaProdutos` abaixo, esta NÃO tem teto — `por_dimensao`
  // (`app/servicos/monitoramento.py`) agrupa por departamento/seção sem
  // `rownum`, porque o universo é o número de departamentos/seções (dezenas),
  // não de produtos (milhares). Conferido no serviço; não chutado.
  const [ordenar, setOrdenar] = useState("atual");
  ...
  const extratores = {
    grupo: (g) => g.grupo, atual: (g) => g.atual, anterior: (g) => g.anterior, variacao: (g) => g.variacao,
  };
  const ordenados = ordenarLista(grupos, extratores[ordenar] ?? extratores.atual, dir);
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-2xs uppercase tracking-wide text-gray-500">
              <CabecalhoOrdenavel {...props} coluna="grupo" padrao="asc" className="px-3">{ROTULO_DIMENSAO[dimensao]}</CabecalhoOrdenavel>
              <CabecalhoOrdenavel {...props} coluna="atual" padrao="desc" align="right" className="px-3">Período</CabecalhoOrdenavel>
              <CabecalhoOrdenavel {...props} coluna="anterior" padrao="desc" align="right" className="px-3">Ano anterior</CabecalhoOrdenavel>
              <CabecalhoOrdenavel {...props} coluna="variacao" padrao="desc" align="right" className="px-3">Variação</CabecalhoOrdenavel>
...
function TabelaProdutos({ itens, fmt, metrica, totalProdutos }) {
  // Mesma lógica de `TabelaGrupos` acima: lista sem paginação, ordenação NO
  // CLIENTE (§3.1). Diferente dela, esta TEM teto: `por_produto`
  // (`app/servicos/monitoramento.py:230-231`) corta `rownum <= 50` DEPOIS de
  // `order by atual desc` — "atual" é a soma da métrica escolhida no
  // período do filtro. Ajuste 3 do revisor (13/09): ordenar no cliente por
  // "Ano anterior", por exemplo, reordena só os 50 já recortados pelo maior
  // do período corrente — não é "o maior do filtro inteiro" nessa outra
  // coluna. Não dá para consertar aqui (o corte é do servidor); o rótulo
  // abaixo diz exatamente o que está sendo ordenado, para a ordenação local
  // não parecer prometer mais do que este recorte contém. `totalProdutos`
  // vem de `resumo().produtos` (`count(distinct codigo_produto)`, SEM
  // `rownum`) — o mesmo número que já aparece no card acima da tabela — e só
  // dispara o aviso quando o corte realmente aconteceu.
  const [ordenar, setOrdenar] = useState("atual");
  const [dir, setDir] = useState("desc");
  const aoOrdenar = (coluna, d) => { setOrdenar(coluna); setDir(d); };
  ...
  const extratores = {
    codigo: (p) => p.codigo, nome: (p) => p.nome, departamento: (p) => p.departamento,
    atual: (p) => p.atual, anterior: (p) => p.anterior, variacao: (p) => p.variacao,
  };
  const ordenados = ordenarLista(itens, extratores[ordenar] ?? extratores.atual, dir);
  const props = { ordenar, dir, aoOrdenar };
  const rotuloMetrica = METRICAS.find((m) => m.id === metrica)?.rotulo ?? "Faturamento";
  return (
    <>
      {totalProdutos > itens.length && (
        <p className="mb-1.5 text-xs text-gray-500">
          Mostrando os {numero(itens.length)} produtos com maior {rotuloMetrica.toLowerCase()} do
          período (de {numero(totalProdutos)} no filtro). Reordenar as colunas abaixo reordena
          só este recorte.
        </p>
      )}
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-2xs uppercase tracking-wide text-gray-500">
              ...
              <CabecalhoOrdenavel {...props} coluna="nome" padrao="asc" className="min-w-[240px]">Produto</CabecalhoOrdenavel>
              <CabecalhoOrdenavel {...props} coluna="departamento" padrao="asc">Departamento</CabecalhoOrdenavel>
              <CabecalhoOrdenavel {...props} coluna="atual" padrao="desc" align="right">Período</CabecalhoOrdenavel>
              <CabecalhoOrdenavel {...props} coluna="anterior" padrao="desc" align="right">Ano anterior</CabecalhoOrdenavel>
              <CabecalhoOrdenavel {...props} coluna="variacao" padrao="desc" align="right">Variação</CabecalhoOrdenavel>
            </tr>
          </thead>
          <tbody>
            {ordenados.map((p) => (
