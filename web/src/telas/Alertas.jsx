...
export default function Alertas() {
  ...
  // clicar em qualquer coluna abaixo troca para ela e tira "prioridade" do
  // estado, exatamente como clicar em qualquer outra coluna faria. Ajuste 1
  // do revisor (13/09): diferente de Pedidos/Precificação, esta tela NÃO tem
  // o `<select>` "Ordenar por" — sem ele, sair de "prioridade" não tinha
  // volta a não ser editar a URL. `resetar` (abaixo do rótulo da lista) é o
  // caminho de volta.
  const { ordenar, dir, aoOrdenar, resetar } = useOrdenacaoUrl("prioridade", null, setPagina);
  ...
  const buscar = useCallback(async () => {
    ...
    try {
      setDados(await api.produtos({
        ...
        comprador: filtroComprador || null,
        status: filtroStatus === "Todos" ? null : filtroStatus,
        busca: busca || null,
        ordenar, dir,
        // Margem/MKP/Custo dependem do cenário fiscal escolhido (mesma regra
        // de Precificacao.jsx) — sem isto, ordenar por "Margem AT" ordenaria
        // sempre pelo cenário ST s/Valor, discordando do que a coluna mostra.
  ...
  }, [ativos, filtroDepartamento, filtroComprador, filtroStatus, busca, ordenar, dir, cenarioSel,
      pagina, versaoDados]);
  ...
  return (
    <div className="pb-6">
      ...
      <div className="flex items-center gap-1 px-4 pt-3 text-xs text-gray-500 md:px-6">
        ...
        Filtrar produto por (pode combinar os quatro)
      </div>
      <div className="grid grid-cols-2 gap-2 px-4 pt-1.5 md:max-w-[820px] md:grid-cols-4 md:px-6">
        ...
        {/* Backend já pronto: GET /api/produtos aceita `comprador`, GET
            /api/opcoes já devolve a lista (item 4 do Diretor, 08/09). */}
        <Campo rotulo="Comprador">
          <Select valor={filtroComprador} vazio="Todos"
                  aoTrocar={(v) => { setFiltroComprador(v); setPagina(1); }}
                  opcoes={opcoes?.compradores ?? []} />
        </Campo>
      ...
      {/* --------------------------------------------------------- lista --- */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 pb-1 pt-4 text-xs font-medium text-gray-500 md:px-6">
        {/* "Prioridade" (severidade → curva ABC → soma) é a ordem padrão da
            tela; clicar numa coluna troca para ela e o rótulo some — a seta
            no cabeçalho da coluna já diz o que está ordenando (§3.3). */}
        <span>
          {ordenar === "prioridade" && (ativos.length === 0
            ? `Prioridade — ${numero(dados?.total)} com alerta`
            : `Prioridade — ${numero(dados?.total)} produto(s)`)}
          {ordenar !== "prioridade" && `${numero(dados?.total)} produto(s)`}
        </span>
        {/* Ajuste 1 do revisor (13/09): "prioridade" é ordem COMPOSTA, não
            coluna (§3.3) — não tem cabeçalho para clicar de volta, e esta
            tela não tem o `<select>` "Ordenar por" de Pedidos/Precificação
            (ela já usa a fileira de filtros para status/tipo/cenário, mais
            cheia que a das outras duas). Um botão que só aparece quando a
            ordem NÃO é a padrão é mais barato aqui do que replicar o
            `<select>` inteiro para uma tela com um único padrão "correto" —
            e resolve o mesmo problema: clicar numa coluna nunca pode ser
            caminho sem volta. */}
        {ordenar !== "prioridade" && (
...
function Lista({ itens, cenarioSel, metaPorTipo, ordenar, dir, aoOrdenar }) {
  const props = { ordenar, dir, aoOrdenar };
  return (
    <>
      {/* mesa: a tabela de 13 colunas. Cabeçalho clicável só nas colunas com
          entrada em `ORDENACOES`/`ORDENACOES_VALIDAS` de `produto.py` (§3.1):
          Classe, Alertas (lista de badges) e as colunas de VAREJO (o servidor
          só resolve margem/MKP do cenário para o ATACADO — `_ordem` fixa
          "atacado" — e Sugerido não tem coluna nenhuma) ficam sem
          `CabecalhoOrdenavel`, de propósito. */}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-2xs uppercase tracking-wide text-gray-500">
                ...
                <CabecalhoOrdenavel {...props} coluna="descricao" padrao="asc" className="min-w-[260px]">Produto</CabecalhoOrdenavel>
                ...
                <CabecalhoOrdenavel {...props} coluna="giro" padrao="desc" align="center">Dias s/venda</CabecalhoOrdenavel>
                <CabecalhoOrdenavel {...props} coluna="cobertura" padrao="asc" align="center">Cobertura</CabecalhoOrdenavel>
                <CabecalhoOrdenavel {...props} coluna="mkp" padrao="asc" align="center" style={{ background: FUNDO_ATACADO }}>MKP AT</CabecalhoOrdenavel>
                <CabecalhoOrdenavel {...props} coluna="margem" padrao="asc" align="center" style={{ background: FUNDO_ATACADO }}>Margem AT</CabecalhoOrdenavel>
                ...
                <CabecalhoOrdenavel {...props} coluna="valorEmRisco" padrao="desc" align="center">Valor em risco</CabecalhoOrdenavel>
              </tr>
            </thead>
            <tbody>
              {itens.map((p) => {
                ...
                return (
                  <tr key={p.codigo} className="border-t border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2">
                      {/* `<Link>`, não `<button onClick>` (Etapa 13, ponto 5): só
                          um elemento com `href` de verdade dá ctrl+clique, clique
                          do meio e "abrir em nova aba" de graça. */}
